// PlayerShot.cpp
//
// Source for the PlayerShot object.
...
PlayerShot::PlayerShot() : PointObject()
{
	Define(player_shot_def);
	SetObjectType(PlayerShotObj);
	SetCollideDamage(PLAYER_SHOT_DAMAGE);
	SetCollisionMass(PLAYER_SHOT_COLLISION_MASS);
	life_timer = PLAYER_SHOT_MAX_AGE;
}

void PlayerShot::Update(BOOL wraparound)
{
	Vect new_pos;
	Vect v(velocity);
